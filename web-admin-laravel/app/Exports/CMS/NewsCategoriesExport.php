...
class NewsCategoriesExport implements FromArray,WithHeadings,ShouldAutoSize, WithEvents, WithTitle, WithStyles
{
    /**
    * @return \Illuminate\Support\Collection
    */
    protected $news_categories;
    public function __construct($news_categories)
    {
    ...
    {
      $data = [];
      foreach ($this->news_categories as $key=>$news_category) {
        if ($news_category->is_active == 1) {
            $is_active = "Active";
        } else {
            $is_active = "Inactive";
        }
        $single = [++$key,$news_category->name,$is_active ];
        $data[] = $single;
      }
      return $data;
    }
