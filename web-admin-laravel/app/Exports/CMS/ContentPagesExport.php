...
class ContentPagesExport implements FromArray,WithHeadings,ShouldAutoSize, WithEvents, WithTitle, WithStyles
{
    /**
    * @return \Illuminate\Support\Collection
    */
    protected $content_pages;
    public function __construct($content_pages)
    {
    ...
    {
      $data = [];
      foreach ($this->content_pages as $key=>$content_page) {
        if ($content_page->is_active == 1) {
            $is_active = "Active";
        } else {
            $is_active = "Inactive";
        }
        $single = [++$key,$content_page->name,$content_page->slug,$is_active ];
        $data[] = $single;
      }
      return $data;
    }
