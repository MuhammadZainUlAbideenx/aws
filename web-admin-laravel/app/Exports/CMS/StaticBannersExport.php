...
class StaticBannersExport implements FromArray,WithHeadings,ShouldAutoSize, WithEvents, WithTitle, WithStyles
{
    /**
    * @return \Illuminate\Support\Collection
    */
    protected $static_banners;
    public function __construct($static_banners)
    {
    ...
    {
        return 'Static Banners';
    }
    public function array(): array
    {
      $data = [];
      foreach ($this->static_banners as $key=>$static_banner) {
        if ($static_banner->is_active == 1) {
            $is_active = "Active";
        } else {
            $is_active = "Inactive";
        }
        $single = [++$key,$static_banner->name,date('d-m-Y', strtotime($static_banner->expiry_date)),$is_active];
        $data[] = $single;
      }
      return $data;
    }
