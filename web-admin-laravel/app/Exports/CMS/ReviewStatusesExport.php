...
class ReviewStatusesExport implements FromArray,WithHeadings,ShouldAutoSize, WithEvents, WithTitle, WithStyles
{
    /**
    * @return \Illuminate\Support\Collection
    */
    protected $review_statuses;
    public function __construct($review_statuses)
    {
        $this->review_statuses = $review_statuses;
    }
    ...
    {
      $data = [];
      foreach ($this->review_statuses as $key=>$review_status) {
        if ($review_status->is_active == 1) {
            $is_active = "Active";
        } else {
            $is_active = "Inactive";
        }
        $single = [++$key,$review_status->name,$is_active ];
        $data[] = $single;
      }
      return $data;
    }
