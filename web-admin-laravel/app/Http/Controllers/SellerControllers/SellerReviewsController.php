...
class SellerReviewsController extends Controller
{
    ...
    public function getter($req = null, $export = null)
    {
        ...
        $reviews = Review::withAllVendor()->orderBy('id', 'desc')->paginate(10);
        $reviews = ReviewsResource::collection($reviews)->response()->getData(true);
        return $reviews;
    }
    ...
    {
        $review = Review::withAllVendor()->find($review);
    ...
    public function activeCustomersSeller()
    {
        $orders_ids = Order::select('customer_id')->where('vendor_id', auth()->user()->id)->get()->unique('customer_id')->toArray();
        $customers = Customer::whereIn('id', $orders_ids)->where('is_active', 1)->get();
