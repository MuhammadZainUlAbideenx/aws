...
class Instamojo extends Controller
{
    ...
    public function generateBody($required,$customer){
      ...
      $this->body['purpose'] = Str::random(10);
      $this->body['amount'] = $required['total'];
    ...
    public function sendRequest(){
      ...
      $res = Http::withHeaders($this->authorization)->asForm()->post('https://api.instamojo.com/v2/payment_requests' , $this->body);
    ...
    public function verifyPayment($params){
      $res = HTTP::asForm()->post('https://api.instamojo.com/oauth2/token/',$this->headers);
        if($res->successful() || $res->ok()){
          ...
          $this->access_token = $token->access_token;
          $this->authorization['Authorization'] = 'Bearer '.$this->access_token;
          $res = Http::withHeaders($this->authorization)->post('https://api.instamojo.com/v2/payments/'.$params['payment_id']);
          return $this->Response($res);
        }else{
          $this->Response($res);
        }
