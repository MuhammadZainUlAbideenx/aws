...
class CreateRequest extends FormRequest
{
      ...
      if($translationRequiredForAllLanguages == 1){
        ...
        foreach ($allLanguages as $language){
          if($language->is_default){
            ...
            $allRules['short_description.'.$language->code] = 'required|string';
            $allRules['description.'.$language->code] = 'required|string';
            $allRules['refund_policy.'.$language->code] = 'string';
          }
        }
      }
    ...
    //   $allRules['sku'] = 'required_if:product_type,1|unique:products';
      ...
      $allRules['flash_sale.start_date'] = 'required_if:flash_sale.exists,1';
      $allRules['flash_sale.expire_date'] = 'required_if:flash_sale.exists,1';
      if($this->flash_sale['exists']){
        $allRules['flash_sale.product_price'] = 'required_if:flash_sale.exists,===,true|lt:price';
      }
      else{
        $allRules['flash_sale.product_price'] = 'required_if:flash_sale.exists,===,true';
      }
      // $allRules['flash_sale.description'] = 'required|string';
      $allRules['special_sale.exists'] = 'required|boolean';
      if($this->special_sale['exists']){
        $allRules['special_sale.special_price'] = 'required_if:special_sale.exists,===,true|lt:price';
      }
      else{
        $allRules['special_sale.special_price'] = 'required_if:special_sale.exists,===,true';
      }
