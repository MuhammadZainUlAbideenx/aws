...
const actions = {
   async fetchReviews({ commit }) {
     ...
     return await this.$repositories.reviews.index()
     .then(
         response => {
           if(response.status)
           {
  ...
  async addReviews({ commit }, requestParams) {
   ...
   return await this.$repositories.reviews.create(requestParams)
   .then(
       response => {
...
async updateReviews({ commit }, {formData,id}) {
   commit('updateReviewRequest',reviews);
    return await this.$repositories.reviews.update(formData,id)
   .then(
       response => {
         if(response.state == 'fail' || response.status)
         {
  ...
  async filterReviews({ commit }, requestParams) {
    return await this.$repositories.reviews.filter(requestParams)
    .then(
        response => {
  ...
  async deleteReviews({ dispatch, commit, state }, {filterData,review_id}) {
     return await this.$repositories.reviews.delete(filterData,review_id)
     .then(
         response => {
           if(response.state == 'fail')
           {
  ...
  return await this.$repositories.reviews.updateStatus(filterData,review_id)
    .then(
      response => {
        if(response.state == 'fail')
        {
  ...
  return await this.$repositories.reviews.updateIsDefault(filterData,review_id)
    .then(
      response => {
        if(response.state == 'fail')
        {
