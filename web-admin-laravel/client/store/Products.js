...
const actions = {
   async fetchProducts({ commit }) {
     ...
     return await this.$repositories.products.index()
     .then(
         response => {
           if(response.status)
           {
  ...
  async addProducts({ commit }, requestParams) {
   ...
   return await this.$repositories.products.create(requestParams)
   .then(
       response => {
...
async updateProducts({ commit }, {formData,id}) {
   commit('updateProductRequest',products);
    return await this.$repositories.products.update(formData,id)
   .then(
       response => {
         if(response.state == 'fail' || response.status)
         {
...
async addProductAttribute({ commit }, {formData,id}) {
   commit('addProductAttributeRequest',products);
    return await this.$repositories.products.addAttribute(formData,id)
   .then(
       response => {
         if(response.state == 'fail' || response.status)
         {
...
async updateProductAttribute({ commit }, {formData,id}) {
   commit('updateProductAttributesRequest',products);
    return await this.$repositories.products.updateAttribute(formData,id)
   .then(
       response => {
         if(response.state == 'fail' || response.status)
         {
...
async updateProductMedia({ commit }, {formData,id}) {
   commit('updateProductMediaRequest',products);
    return await this.$repositories.products.updateMedia(formData,id)
   .then(
       response => {
         if(response.state == 'fail' || response.status)
         {
  ...
  async filterProducts({ commit }, requestParams) {
    return await this.$repositories.products.filter(requestParams)
    .then(
        response => {
  ...
  async filterVendorProducts({ commit }, requestParams) {
    return await this.$generalService.filterVendorProducts({vendor_id:requestParams})
    .then(
        response => {
          if(response.state == 'fail')
          {
            commit('filterVendorProductsFailure',  response.error);
          }
          else if(response.products){
            commit('filterVendorProductsSuccess', response.products);
          }
          return response
        },
        error => {
  ...
  async deleteProducts({ dispatch, commit, state }, {filterData,product_id}) {
     return await this.$repositories.products.delete(filterData,product_id)
     .then(
         response => {
           if(response.state == 'fail')
           {
  ...
  return await this.$repositories.products.updateStatus(filterData,product_id)
    .then(
      response => {
        if(response.state == 'fail')
        {
...
updateProductAttributesFailure(state) {
  ...
  filterVendorProductsSuccess(state, products) {
    state.status = { filtered: true };
    state.products = products;
},
filterVendorProductsFailure(state) {
    state.status = {};
},
