...
const actions = {
   async fetchOrders({ commit }) {
     ...
     return await this.$repositories.orders.index()
     .then(
         response => {
           if(response.status)
           {
  ...
  async addOrders({ commit }, requestParams) {
   ...
   return await this.$repositories.orders.create(requestParams)
   .then(
       response => {
...
async updateOrders({ commit }, {formData,id}) {
   commit('updateOrderRequest',orders);
    return await this.$repositories.orders.update(formData,id)
   .then(
       response => {
         if(response.state == 'fail' || response.status)
         {
  ...
  async filterOrders({ commit }, requestParams) {
    return await this.$repositories.orders.filter(requestParams)
    .then(
        response => {
  ...
  async deleteOrders({ dispatch, commit, state }, {filterData,order_id}) {
     return await this.$repositories.orders.delete(filterData,order_id)
     .then(
         response => {
           if(response.state == 'fail')
           {
  ...
  return await this.$repositories.orders.updateStatus(filterData,order_id)
    .then(
      response => {
        if(response.state == 'fail')
        {
  ...
  return await this.$repositories.orders.updateIsDefault(filterData,order_id)
    .then(
      response => {
        if(response.state == 'fail')
        {
