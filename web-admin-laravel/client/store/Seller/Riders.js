...
const actions = {
   async fetchRiders({ commit }) {
     ...
     return await this.$sellerRepositories.riders.index()
     .then(
         response => {
           if(response.status)
           {
  ...
  async addRiders({ commit }, requestParams) {
   ...
   return await this.$sellerRepositories.riders.create(requestParams)
   .then(
       response => {
...
async updateRiders({ commit }, {formData,id}) {
   commit('updateRiderRequest',riders);
    return await this.$sellerRepositories.riders.update(formData,id)
   .then(
       response => {
         if(response.state == 'fail' || response.status)
         {
  ...
  async filterRiders({ commit }, requestParams) {
    return await this.$sellerRepositories.riders.filter(requestParams)
    .then(
        response => {
  ...
  async deleteRiders({ dispatch, commit, state }, {filterData,rider_id}) {
     return await this.$sellerRepositories.riders.delete(filterData,rider_id)
     .then(
         response => {
           if(response.state == 'fail')
           {
  ...
  return await this.$sellerRepositories.riders.updateStatus(filterData,rider_id)
    .then(
      response => {
        if(response.state == 'fail')
        {
  ...
  return await this.$sellerRepositories.riders.updateIsDefault(filterData,rider_id)
    .then(
      response => {
        if(response.state == 'fail')
        {
