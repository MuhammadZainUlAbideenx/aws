...
const actions = {
   async fetchRiderPayoutRequests({ commit }) {
     ...
     return await this.$sellerRepositories.riderPayoutRequests.index()
     .then(
         response => {
           if(response.status)
           {
  ...
  async filterRiderPayoutRequests({ commit }, requestParams) {
    return await this.$sellerRepositories.riderPayoutRequests.filter(requestParams)
    .then(
        response => {
  ...
  return await this.$generalService.updateSellerRiderPayoutRequestStatus(filterData,riderPayoutRequest_id,status)
    .then(
      response => {
        if(response.state == 'fail')
        {
