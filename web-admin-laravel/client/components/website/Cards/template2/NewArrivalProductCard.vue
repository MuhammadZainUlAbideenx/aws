<template>
   <div class="card m-0 p-3 bg-transparent h-100 border-1">
              <div class="p-0 bg-light rounded position-relative mb-3 card-img overflow-hidden">
                <img
                v-if="product.media.length>0"
                 :src="`/backend${product.media[0].thumbnails.large.thumbnail}`"
                  class="w-100 rounded card-image"
                  alt=""
                />
                <img
                v-else
                  src="~/assets/images/defult-product-img.png"
                  class="w-100 rounded card-image-1"
                  alt=""
                />
                 ...
                 <div v-if="product.tags && product.tags.length > 0" class="d-flex flex-column justify-content-end align-items-end position-absolute badge-custom">
                    <span v-for="(tag,index) in product.tags" :key="index" class="d-block mb-1">
              ...
              <div class="text-section">
               ...
               <span v-if="product.categories && product.categories.length > 0">
                ...
                <span class="w-100 d-block mb-3 text-dark-50" v-if="product.flash_sale">{{product.display_price}}<small class="text-danger"> {{product.flash_sale.display_price}}</small></span>
                <span class="w-100 d-block mb-3 text-dark-50" v-else-if="product.special_sale">{{product.display_price}}<small class="text-danger"> {{product.special_sale.display_price}}</small></span>
                <!-- <span class="w-100 d-block mb-3 text-dark-50" v-else>{{product.display_price}}<small class="text-danger"> {{product.special_sale.display_price}}</small></span> -->
                    <span class="product-price d-flex align-items-center mb-3" v-else>
                    <span v-if="product.attributes &&  product.attributes.length > 0">
                        {{ $t("starting_from_price") }}
                    <span class="w-100 text-dark-50" v-if="product.attributes &&  product.attributes.length > 0">
                    {{ product.starting_from_price }}
                    </span>
                 </span>
                <span v-else>
                    <span class="w-100 d-block text-dark-50">
                    {{ product.display_price }}
                    </span>
                </span>
                    </span>
                <div class="d-flex align-items-center">
                  <button type="button" @click="addToCart(product)" class="btn btn-primary border-0 text-uppercase">{{$t('web.home.new_arrival_products.add_to_cart')}}</button>
                  <span>
                        <a
                             href="#"
