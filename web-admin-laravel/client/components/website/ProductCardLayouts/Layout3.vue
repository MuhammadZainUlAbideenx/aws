<template>
  <div class="card_layout_three pt-4">
    <div class="card m-0 p-0 pb-0 mb-5 bg-transparent h-100 border-0 rounded-0 overflow-hidden pe-3 ps-3">
      <div
        class="
          p-0
          bg-light
      ...
      >
        ...
        <img
          ...
          alt=""
        />
        <span class="position-absolute tool-tip">
            <a
              href="#"
          ...
          <button
            ...
            class="btn btn-primary border-0 text-uppercase btn-hover w-100 rounded-0 d-flex justify-content-center"
          >
      ...
      <div class="text-section ps-2 text-center d-flex align-items-center justify-content-center flex-column bg-secondary pt-3">
        <div class="rating mb-1 text-center text-dark star-dark">
                   <GlobalRating :rating="product.reviews_average_rating" />
                 </div>
        <h3>
          <nuxt-link :to="`/product/${product.slug}`" class="fs-5 fw-bold">{{
            product.name
          }}</nuxt-link>
        </h3>
        ...
        <span class="w-100 d-block mb-3 text-dark-50" v-if="product.flash_sale"
          ><span class="text-decoration-line-through">
              {{ product.display_price }}
            </span><small class="text-danger text-center">
            {{ product.flash_sale.display_price }}</small
          ></span
        >
        <span
          class="w-100 d-block mb-3 text-dark-50"
          v-else-if="product.special_sale"
          ><span class="text-decoration-line-through">
              {{ product.display_price }}
            </span><small class="text-danger">
            {{ product.special_sale.display_price }}</small
          ></span
        >
