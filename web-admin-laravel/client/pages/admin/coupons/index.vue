    ...
    <div class="content-header">
      <div class="row g-0">
        <div class="col-sm-12 px-0">
          <h2 class="m-0 text-body bold">{{ this.$t("sidebar.coupon") }}</h2>
        </div>
        <!-- /.col -->
        <div class="row">
          <div class="col-sm-12 px-0">
            ...
            <p class="text-body-secondary">
              {{ this.$t("form.coupon.index_description") }}
            </p>
          </div>
          <!-- /.col -->
        </div>
              ...
              <div class="col-lg-8">
                <div class="d-flex flex-column flex-md-row">
                  ...
                  >
                    ...
                    <option value="name">{{ $t("columns.code") }}</option>
                  </select>
              ...
              <div class="col-lg-4 table-actions px-0">
                <div slot="table-actions" class="align-middle">
                  ...
                  <nuxt-link
                    ...
                    :to="localePath('/admin/coupons/create')"
                  >
                    <button
                      type="button"
                      v-tooltip="{ content: 'Add Coupon' }"
            ...
            <div class="col-md-12">
              ...
              >
                <template slot="table-row" slot-scope="props">
                  <div v-if="props.column.field == 'is_active'">
                    <div class="form-switch">
                      <input
                        ...
                        @change="
                          (event) => changeCouponStatus(props.row.id, event)
                        "
                  ...
                  <div v-else-if="props.column.field == 'individual_use'">
                    <div class="form-switch">
                      <input
                        class="form-check-input"
                        @change="
                        ...
                        "
                        :checked="
                          props.row.individual_use == 1 ? 'checked' : ''
                        "
                        type="checkbox"
                        id="flexSwitchCheckChecked"
                      />
                  ...
                  <div v-else-if="props.column.field == 'amount_index'">
                    <div v-if="props.row.discount_type == 1">
                      {{ props.row.amount_index }}
                    </div>
                    <div v-if="props.row.discount_type == 2">
                      {{ props.row.amount }}%
                    </div>
                  </div>
                  <div v-else-if="props.column.field == 'exclude_sale_items'">
                    <div class="form-switch">
                      <input
                        class="form-check-input"
                        @change="
                        ...
                        "
                        :checked="
                          props.row.exclude_sale_items == 1 ? 'checked' : ''
                        "
                        type="checkbox"
                        id="flexSwitchCheckChecked"
                      />
                  ...
                  <div v-else-if="props.column.field == 'actions'">
                    <nuxt-link
                      ...
                      :to="localePath('/admin/coupons/view/' + props.row.id)"
                    >
                      <fa :icon="['fas', 'eye']" fixed-width />
                    </nuxt-link>
                    <nuxt-link
                      v-permission="'coupons.update'"
                      v-tooltip="{ content: 'Edit' }"
                      class="btn btn-sm rounded-circle text-success"
                      :to="localePath('/admin/coupons/edit/' + props.row.id)"
                    >
                    ...
                    <AdminDeleteModal
                      ...
                      @deleteClicked="deleteCoupon(props.row.id)"
...
export default {
  ...
  data() {
    return {
      columns: [
        ...
        //   },
        {
          label: this.$t("columns.exclude_sale_items"),
          field: "exclude_sale_items",
        },
        {
          label: this.$t("columns.expiry_date"),
          field: "display_expiry_date",
        },
        {
  ...
  async fetch() {
    if (this.$gates.hasPermission("coupons.index")) {
      await this.fetchCoupons(this.page).then();
    }
  },
  methods: {
    ...mapActions({
      ...
      filterCoupons: "Coupons/filterCoupons",
      updateCouponStatus: "Coupons/updateCouponStatus",
    }),
    async exportCoupons(type) {
      this.filterData.export = type;
      await this.$repositories.coupons.export(this.filterData);
    },
    async deleteCoupon(id) {
      await this.deleteCoupons({
    ...
    async filter() {
      await this.filterCoupons(this.filterData).then((response) => {
        if (response.state == "fail") {
          this.$toast.error(response.message);
        }
        if (response.state == "success") {
    ...
    async changeCouponStatus(id, event) {
  ...
  mounted() {},
  watch: {
    allCoupons(newCount, oldCount) {
      if (
        this.allCoupons &&
        this.allCoupons != null &&
        this.allCoupons.data != null
      ) {
        this.rows = this.allCoupons.data;
        this.totalRows = this.allCoupons.meta.total;
        if (this.filterData.page != this.allCoupons.meta.current_page) {
          this.filterData.page = this.allCoupons.meta.current_page;
        }
        if (this.filterData.perPage != this.allCoupons.meta.per_page) {
          this.filterData.perPage = this.allCoupons.meta.per_page;
        }
      }
    },
  },
