    ...
    <div class="content-header">
        <div class="row g-0">
          <div class="col-sm-12 px-0">
            <h2 class="m-0 text-body bold">{{this.$t("sidebar.news")}}</h2>
          </div>
          <!-- /.col -->
          <div class="row">
            <div class="col-sm-12 px-0">
              <ol class="breadcrumb float-sm-right text-body">
                ...
                <li class="breadcrumb-item active">{{this.$t("sidebar.news")}}</li>
              </ol>
              <p class="text-body-secondary">{{this.$t("form.news.index_description")}}</p>
            </div>
            <!-- /.col -->
          </div>
        </div>
             ...
             <div class="col-lg-8">
               ...
               <div class="col-lg-4 table-actions px-0">
                <div slot="table-actions" class="align-middle">
                  ...
                  <nuxt-link
                    v-permission="'newses.create'"
                    :to="localePath('/admin/news/create')"
                  >
                    <button
                      type="button"
                      v-tooltip="{ content: 'Add News' }"
                  ...
                  <div v-if="props.column.field == 'is_active'">
                    <div class="form-switch">
                      <input
                        ...
                        @change="
                          (event) => changeNewsStatus(props.row.id, event)
                        "
                   ...
                   <div v-else-if="props.column.field == 'actions'">
                    <nuxt-link
                            v-permission="'newses.index'" v-tooltip="{ content: 'Show' }"
                              class="btn btn-sm rounded-circle text-primary   "
                            :to="
                              localePath('/admin/news/view/' + props.row.id)
                            "
                            >
                          ...
                          >
                          <nuxt-link
                          v-permission="'newses.update'" v-tooltip="{ content: 'Edit' }"
                            class=" btn btn-sm rounded-circle text-success   "
                          :to="
                            localePath('/admin/news/edit/' + props.row.id)
                          "
                          >
                            <fa :icon="['fas', 'edit']" fixed-width />
                          </nuxt-link>
                       <button
                        v-permission="'newses.delete'"
                        ...
                        >
                         <fa :icon="['fas', 'trash-alt']" fixed-width />
                                          ...
                                          <AdminDeleteModal :modal_id="'DeleteItem'+props.row.id" @deleteClicked="deleteNews(props.row.id)"  :delete_id="props.row.id">
                </AdminDeleteModal>
                  </div>
                  <span v-else>
...
<script >
  ...
  export default {
    ...
    async fetch() {
      if (this.$gates.hasPermission('newses.index')) {
        await this.fetchNewses(this.page).then()

      }
    },
    methods: {
      ...mapActions({
        fetchNewses: 'Newses/fetchNewses',
        fetchActiveNewses: 'General/fetchActiveNewses',
        deleteNewses: 'Newses/deleteNewses',
        filterNewses: 'Newses/filterNewses',
        updateNewsStatus: 'Newses/updateNewsStatus',
      }),
      async exportNewses(type) {
        this.filterData.export = type
        await this.$repositories.news.export(this.filterData)
      },
    async deleteNews(id) {
      await this.deleteNewses({filterData: this.filterData,news_id: id,}).then((response) => {
      ...
      async filter() {
        await this.filterNewses(this.filterData).then(response => {
          if (response.state == 'fail') {
            this.$toast.error(response.message)
          }
          if (response.state == 'success') {
    ...
    async changeNewsStatus(id, event) {
      await this.updateNewsStatus({
    ...
    computed: {
      ...mapGetters({
        allNewses: 'Newses/allNewses'
      })
    },
    mounted() {
    },
    watch: {
      allNewses(newCount, oldCount) {
        if (this.allNewses && this.allNewses != null && this.allNewses.data != null)   {
          this.rows = this.allNewses.data
          this.totalRows = this.allNewses.meta.total
          if (this.filterData.page != this.allNewses.meta.current_page) {
            this.filterData.page = this.allNewses.meta.current_page
          }
          if (this.filterData.perPage != this.allNewses.meta.per_page) {
            this.filterData.perPage = this.allNewses.meta.per_page
          }
        }
      }
    }
