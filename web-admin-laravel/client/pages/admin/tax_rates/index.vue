    ...
    <div class="content-header">
        <div class="row g-0">
          <div class="col-sm-12 px-0">
            <h2 class="m-0 text-body bold">{{this.$t("sidebar.tax_rate")}}</h2>
          </div>
          <!-- /.col -->
          <div class="row">
            <div class="col-sm-12 px-0">
              <ol class="breadcrumb float-sm-right text-body">
                ...
                <li class="breadcrumb-item active">{{this.$t("sidebar.tax_rate")}}</li>
              </ol>
              <p class="text-body-secondary">{{this.$t("form.tax_rate.index_description")}}</p>
            </div>
            <!-- /.col -->
          </div>
        </div>
                ...
                <div slot="table-actions" class="align-middle">
                  <div class="share">
                    <nav>
                      ...
                      <span @click="exportTaxRates('xlsx')">
                        <nuxt-link
                          to="#"
                  ...
                  <nuxt-link
                    v-permission="'tax_rates.create'"
                    :to="localePath('/admin/tax_rates/create')"
                  >
                    <button
                      type="button"
                      v-tooltip="{ content: 'Add Tax Rate' }"
                ...
                <div v-if="props.column.field == 'is_active'">
                  <div class="form-switch">
                    <input
                      ...
                      @change="
                        (event) => changeTaxRateStatus(props.row.id, event)
                      "
                    ...
                    <nuxt-link
                            v-permission="'tax_rates.index'"  v-tooltip="{ content: 'Show' }"
                              class="btn btn-sm rounded-circle text-primary   "
                            :to="
                              localePath('/admin/tax_rates/view/' + props.row.id)
                            "
                            >
                          ...
                          >
                          <nuxt-link
                          v-permission="'tax_rates.update'"  v-tooltip="{ content: 'Edit' }"
                            class=" btn btn-sm rounded-circle text-success   "
                          :to="
                            localePath('/admin/tax_rates/edit/' + props.row.id)
                          "
                          >
                            <fa :icon="['fas', 'edit']" fixed-width />
                         </nuxt-link>
                       <button
                        v-permission="'tax_rates.delete'"
                    ...
                    <AdminDeleteModal :modal_id="'DeleteItem'+props.row.id" @deleteClicked="deleteTaxRate(props.row.id)"  :delete_id="props.row.id">
                </AdminDeleteModal>
                  </div>
                  <span v-else>
...
<script >
  ...
  export default {
    ...
    data() {
      return {
        columns: [
          ...
          {
            label: this.$t('columns.zone'),
            field: 'zone.name',
            sortable: false
          },
          {
            label: this.$t('columns.tax_class'),
            field: 'tax_class.name',
          },
          {
    ...
    async fetch() {
      if (this.$gates.hasPermission('tax_rates.index')) {
        await this.fetchTaxRates(this.page).then()
      }
    },
    methods: {
      ...mapActions({
        fetchTaxRates: 'TaxRates/fetchTaxRates',
        fetchActiveTaxRates: 'General/fetchActiveTaxRates',
        deleteTaxRates: 'TaxRates/deleteTaxRates',
        filterTaxRates: 'TaxRates/filterTaxRates',
        updateTaxRateStatus: 'TaxRates/updateTaxRateStatus',
        updateTaxRateIsDefault: 'TaxRates/updateTaxRateIsDefault'
      }),
      async exportTaxRates(type) {
        this.filterData.export = type
        await this.$repositories.tax_rates.export(this.filterData)
      },
    async deleteTaxRate(id) {
      await this.deleteTaxRates({
      ...
      async filter() {
        await this.filterTaxRates(this.filterData).then(response => {
          if (response.state == 'fail') {
            this.$toast.error(response.message)
          }
          if (response.state == 'success') {
      ...
      async changeDefaultTaxRate(id, event) {
        await this.updateTaxRateIsDefault({
          filterData: this.filterData,
          tax_rate_id: id
        }).then(response => {
          if (response.state == 'fail') {
    ...
    async changeTaxRateStatus(id, event) {
      await this.updateTaxRateStatus({
        filterData: this.filterData,
        tax_rate_id: id,
      }).then((response) => {
        if (response.state == "fail") {
    ...
    mounted() {},
    watch: {
      allTaxRates(newCount, oldCount) {
        if (this.allTaxRates && this.allTaxRates != null && this.allTaxRates.data != null)   {
          this.rows = this.allTaxRates.data
          this.totalRows = this.allTaxRates.meta.total
          if (this.filterData.page != this.allTaxRates.meta.current_page) {
            this.filterData.page = this.allTaxRates.meta.current_page
          }
          if (this.filterData.perPage != this.allTaxRates.meta.per_page) {
            this.filterData.perPage = this.allTaxRates.meta.per_page
          }
        }
      }
    }
