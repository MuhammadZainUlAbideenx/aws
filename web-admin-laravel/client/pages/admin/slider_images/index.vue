        ...
        <div class="row">
          <div class="col-sm-12 px-0">
            ...
            <p class="text-body-secondary">
              {{ this.$t("form.slider_image.index_description") }}
            </p>
          </div>
          <!-- /.col -->
        </div>
            ...
            <div class="row table-filter-row g-0 justify-content-end">
              ...
              <div class="col-lg-4 table-actions px-0">
                <div slot="table-actions" class="align-middle">
                  <div class="share">
                    <nav>
                      <span @click="exportSliderImages('csv')">
                        <nuxt-link
                          to="#"
                      ...
                      <span @click="exportSliderImages('xlsx')">
                        <nuxt-link
                          to="#"
                      ...
                      <span @click="exportSliderImages('pdf')">
                        <nuxt-link
                          to="#"
                  ...
                  <nuxt-link
                    v-permission="'slider_images.create'"
                    :to="localePath('/admin/slider_images/create')"
                  >
                    <button
                      type="button"
                      v-tooltip="{ content: 'Add Slider Image' }"
                ...
                <template slot="table-row" slot-scope="props">
                  <div v-if="props.column.field == 'image'">
                    <img
                      ...
                      "
                      v-bind:src="
                        url + `${props.row.image.thumbnails.small.thumbnail}`
                      "
                      alt=""
                      height="40px"
                      width="40px"
                  ...
                  <div v-else-if="props.column.field == 'is_active'">
                    <div class="form-switch">
                      <input
                        ...
                        @change="
                          (event) =>
                            changeSliderImageStatus(props.row.id, event)
                        "
                  ...
                  >
                    <nuxt-link
                      v-permission="'slider_images.index'"
                      v-tooltip="{ content: 'Show' }"
                      class="btn btn-sm rounded-circle text-primary"
                      :to="
                        localePath('/admin/slider_images/view/' + props.row.id)
                      "
                    >
                      <fa :icon="['fas', 'eye']" fixed-width />
                    </nuxt-link>
                    <nuxt-link
                      v-permission="'slider_images.update'"
                      v-tooltip="{ content: 'Edit' }"
                      class="btn btn-sm rounded-circle text-success"
                      :to="
                        localePath('/admin/slider_images/edit/' + props.row.id)
                      "
                    >
                      <fa :icon="['fas', 'edit']" fixed-width />
                    </nuxt-link>
                    <button
                      v-permission="'slider_images.delete'"
                      type="button"
                    ...
                    <AdminDeleteModal
                      ...
                      @deleteClicked="deleteSliderImage(props.row.id)"
...
export default {
  ...
  meta: {
    permission: "slider_images.index",
    strategy: "read",
  },
  ...
  async fetch() {
    if (this.$gates.hasPermission("slider_images.index")) {
      await this.fetchSliderImages(this.page).then();
    }
  },
  methods: {
    ...mapActions({
      fetchSliderImages: "SliderImages/fetchSliderImages",
      fetchActiveSliderImages: "General/fetchActiveSliderImages",
      deleteSliderImages: "SliderImages/deleteSliderImages",
      filterSliderImages: "SliderImages/filterSliderImages",
      updateSliderImageStatus: "SliderImages/updateSliderImageStatus",
    }),
    async exportSliderImages(type) {
      this.filterData.export = type;
      await this.$repositories.slider_images.export(this.filterData);
    },
    async deleteSliderImage(id) {
      await this.deleteSliderImages({
    ...
    async filter() {
      await this.filterSliderImages(this.filterData).then((response) => {
        if (response.state == "fail") {
          this.$toast.error(response.message);
        }
        if (response.state == "success") {
    ...
    async changeSliderImageStatus(id, event) {
      await this.updateSliderImageStatus({
        filterData: this.filterData,
        slider_image_id: id,
      })
        .then((response) => {
  ...
  computed: {
    ...mapGetters({
      allSliderImages: "SliderImages/allSliderImages",
    }),
  },
  mounted() {},
  watch: {
    allSliderImages(newCount, oldCount) {
      if (
        this.allSliderImages &&
        this.allSliderImages != null &&
        this.allSliderImages.data != null
      ) {
        this.rows = this.allSliderImages.data;
        this.totalRows = this.allSliderImages.meta.total;
        if (this.filterData.page != this.allSliderImages.meta.current_page) {
          this.filterData.page = this.allSliderImages.meta.current_page;
        }
        if (this.filterData.perPage != this.allSliderImages.meta.per_page) {
          this.filterData.perPage = this.allSliderImages.meta.per_page;
        }
      }
    },
  },
