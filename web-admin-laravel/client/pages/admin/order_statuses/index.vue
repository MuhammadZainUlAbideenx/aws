        ...
        <div class="row">
          <div class="col-sm-12 px-0">
            ...
            <p class="text-body-secondary">
              {{ this.$t("order_status.index_description") }}
            </p>
          </div>
          <!-- /.col -->
        </div>
            ...
            <div class="row table-filter-row g-0">
              ...
              <div class="col-lg-4 table-actions px-0">
                <div slot="table-actions" class="align-middle">
                  <div class="share">
                    <nav>
                      <span @click="exportOrderStatuses('csv')">
                        <nuxt-link
                          to="#"
                      ...
                      <span @click="exportOrderStatuses('xlsx')">
                        <nuxt-link
                          to="#"
                      ...
                      <span @click="exportOrderStatuses('pdf')">
                        <nuxt-link
                          to="#"
                  ...
                  <nuxt-link
                    v-permission="'order_statuses.create'"
                    :to="localePath('/admin/order_statuses/create')"
                  >
                    <button
                      type="button"
                      v-tooltip="{ content: 'Add Order Status' }"
              ...
              >
                <template slot="table-row" slot-scope="props">
                  <div v-if="props.column.field == 'is_active'">
                    <div class="form-switch">
                      <input
                        ...
                        @change="
                          (event) =>
                            changeOrderStatusStatus(props.row.id, event)
                        "
                  ...
                  <div v-else-if="props.column.field == 'is_default'">
                    <div class="form-switch">
                      <input
                        ...
                        @change="
                          (event) =>
                            changeDefaultOrderStatus(props.row.id, event)
                        "
                  ...
                  <div v-else-if="props.column.field == 'actions'">
                    <nuxt-link
                      v-permission="'order_statuses.index'"
                      v-tooltip="{ content: 'Show' }"
                      class="btn btn-sm rounded-circle text-primary"
                      :to="
                        localePath('/admin/order_statuses/view/' + props.row.id)
                      "
                    >
                      <fa :icon="['fas', 'eye']" fixed-width />
                    </nuxt-link>
                    <nuxt-link
                      v-permission="'order_statuses.update'"
                      v-tooltip="{ content: 'Edit' }"
                      class="btn btn-sm rounded-circle text-success"
                      :to="
                        localePath('/admin/order_statuses/edit/' + props.row.id)
                      "
                    >
                      <fa :icon="['fas', 'edit']" fixed-width />
                    </nuxt-link>
                    <button
                      v-permission="'order_statuses.delete'"
                      type="button"
                    ...
                    <AdminDeleteModal
                      ...
                      @deleteClicked="deleteOrderStatus(props.row.id)"
...
export default {
  ...
  meta: {
    permission: "order_statuses.index",
    strategy: "read",
  },
  ...
  data() {
    return {
      columns: [
        ...
        {
          label: this.$t("columns.status_code"),
          field: "status_code",
        },
        {
  ...
  async fetch() {
    if (this.$gates.hasPermission("order_statuses.index")) {
      await this.fetchOrderStatuses(this.page).then();
    }
  },
  methods: {
    ...mapActions({
      fetchOrderStatuses: "OrderStatuses/fetchOrderStatuses",
      fetchActiveOrderStatuses: "General/fetchActiveOrderStatuses",
      deleteOrderStatuses: "OrderStatuses/deleteOrderStatuses",
      filterOrderStatuses: "OrderStatuses/filterOrderStatuses",
      updateOrderStatusStatus: "OrderStatuses/updateOrderStatusStatus",
      updateOrderStatusIsDefault: "OrderStatuses/updateOrderStatusIsDefault",
    }),
    async exportOrderStatuses(type) {
      this.filterData.export = type;
      await this.$repositories.order_statuses.export(this.filterData);
    },
    async deleteOrderStatus(id) {
      await this.deleteOrderStatuses({
    ...
    async filter() {
      await this.filterOrderStatuses(this.filterData).then((response) => {
        if (response.state == "fail") {
          this.$toast.error(response.message);
        }
        if (response.state == "success") {
    ...
    async changeOrderStatusStatus(id, event) {
      await this.updateOrderStatusStatus({
        filterData: this.filterData,
        order_status_id: id,
      })
        .then((response) => {
    ...
    async changeDefaultOrderStatus(id, event) {
      await this.updateOrderStatusIsDefault({
        filterData: this.filterData,
        order_status_id: id,
      }).then((response) => {
        if (response.state == "fail") {
  ...
  computed: {
    ...mapGetters({
      allOrderStatuses: "OrderStatuses/allOrderStatuses",
    }),
  },
  mounted() {},
  watch: {
    allOrderStatuses(newCount, oldCount) {
      if (
        this.allOrderStatuses &&
        this.allOrderStatuses != null &&
        this.allOrderStatuses.data != null
      ) {
        this.rows = this.allOrderStatuses.data;
        this.totalRows = this.allOrderStatuses.meta.total;
        if (this.filterData.page != this.allOrderStatuses.meta.current_page) {
          this.filterData.page = this.allOrderStatuses.meta.current_page;
        }
        if (this.filterData.perPage != this.allOrderStatuses.meta.per_page) {
          this.filterData.perPage = this.allOrderStatuses.meta.per_page;
        }
      }
    },
  },
