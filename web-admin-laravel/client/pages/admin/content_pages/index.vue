          ...
          <div class="row">
            <div class="col-sm-12 px-0">
              ...
              <p class="text-body-secondary">
                {{ this.$t("form.content_page.index_description") }}
              </p>
            </div>
            <!-- /.col -->
          </div>
            ...
            <div class="col-lg-8">
               ...
               <div class="col-lg-4 table-actions px-0">
                <div slot="table-actions" class="align-middle">
                  <div class="share">
                    <nav>
                      <span @click="exportContentPages('csv')">
                        <nuxt-link
                          to="#"
                      ...
                      <span @click="exportContentPages('xlsx')">
                        <nuxt-link
                          to="#"
                      ...
                      <span @click="exportContentPages('pdf')">
                        <nuxt-link
                          to="#"
                  ...
                  <nuxt-link
                    v-permission="'content_pages.create'"
                    :to="localePath('/admin/content_pages/create')"
                  >
                    <button
                      type="button"
                  ...
                  <div v-else-if="props.column.field == 'is_active'">
                    <div class="form-switch">
                      <input
                        ...
                        @change="
                          (event) =>
                            changeContentPageStatus(props.row.id, event)
                        "
                   ...
                   <div v-else-if="props.column.field == 'actions'">
                    <nuxt-link
                            v-permission="'content_pages.index'"  v-tooltip="{ content: 'Show' }"
                              class="btn btn-sm rounded-circle text-primary   "
                            :to="
                              localePath('/admin/content_pages/view/' + props.row.id)
                            "
                            >
                          ...
                          >
                          <nuxt-link
                          v-permission="'content_pages.update'"  v-tooltip="{ content: 'Edit' }"
                            class=" btn btn-sm rounded-circle text-success   "
                          :to="
                            localePath('/admin/content_pages/edit/' + props.row.id)
                          "
                          >
                            <fa :icon="['fas', 'edit']" fixed-width />
                         </nuxt-link>
                       <button
                        v-permission="'content_pages.delete'"
                        ...
                        >
                         <fa :icon="['fas', 'trash-alt']" fixed-width />
                                           ...
                                           <AdminDeleteModal :modal_id="'DeleteItem'+props.row.id" @deleteClicked="deleteContentPage(props.row.id)"  :delete_id="props.row.id">
                </AdminDeleteModal>
                  </div>
                  <span v-else>
...
export default {
  ...
  async fetch() {
    if (this.$gates.hasPermission("content_pages.index")) {
      await this.fetchContentPages(this.page).then();
    }
  },
  methods: {
    ...mapActions({
      fetchContentPages: "ContentPages/fetchContentPages",
      fetchActiveContentPages: "General/fetchActiveContentPages",
      deleteContentPages: "ContentPages/deleteContentPages",
      filterContentPages: "ContentPages/filterContentPages",
      updateContentPageStatus: "ContentPages/updateContentPageStatus",
    }),
    async exportContentPages(type) {
      this.filterData.export = type;
      await this.$repositories.content_pages.export(this.filterData);
    },
    async deleteContentPage(id) {
      await this.deleteContentPages({
        ...
        .then((response) => {
          ...
          if (response.state == "success") {
            ...
            this.fetchActiveContentPages();
          }
        })
        .catch((e) => {});
    },
    async filter() {
      await this.filterContentPages(this.filterData).then((response) => {
        if (response.state == "fail") {
          this.$toast.error(response.message);
        }
        if (response.state == "success") {
    ...
    async changeContentPageStatus(id, event) {
      await this.updateContentPageStatus({
        filterData: this.filterData,
        content_page_id: id,
      })
        .then((response) => {
  ...
  computed: {
    ...mapGetters({
      allContentPages: "ContentPages/allContentPages",
    }),
  },
  mounted() {},
  watch: {
    allContentPages(newCount, oldCount) {
      if (
        this.allContentPages &&
        this.allContentPages != null &&
        this.allContentPages.data != null
      ) {
        this.rows = this.allContentPages.data;
        this.totalRows = this.allContentPages.meta.total;
        if (this.filterData.page != this.allContentPages.meta.current_page) {
          this.filterData.page = this.allContentPages.meta.current_page;
        }
        if (this.filterData.perPage != this.allContentPages.meta.per_page) {
          this.filterData.perPage = this.allContentPages.meta.per_page;
        }
      }
    },
  },
