    ...
    <div class="content-header">
      <div class="row g-0">
        <div class="col-sm-12 px-0">
          <h2 class="m-0 text-body bold">{{ this.$t("sidebar.country") }}</h2>
        </div>
        <!-- /.col -->
        <div class="row">
          <div class="col-sm-12 px-0">
            <ol class="breadcrumb float-sm-right text-body">
              ...
              <li class="breadcrumb-item active">
                {{ this.$t("sidebar.country") }}
              </li>
            </ol>
            <p class="text-body-secondary">
              {{ this.$t("form.country.index_description") }}
            </p>
          </div>
          <!-- /.col -->
        </div>
            ...
            <div class="row table-filter-row g-0">
              ...
              <div class="col-lg-4 table-actions px-0">
                <div slot="table-actions" class="align-middle">
                  <div class="share">
                    <nav>
                      <span @click="exportCountries('csv')">
                        <nuxt-link
                          to="#"
                      ...
                      <span @click="exportCountries('xlsx')">
                        <nuxt-link
                          to="#"
                      ...
                      <span @click="exportCountries('pdf')">
                        <nuxt-link
                          to="#"
                  ...
                  <nuxt-link
                    v-permission="'countries.create'"
                    :to="localePath('/admin/countries/create')"
                  >
                    <button
                      type="button"
                      v-tooltip="{ content: 'Add Country' }"
                ...
                <template slot="table-row" slot-scope="props">
                  <div v-if="props.column.field == 'is_active'">
                    <div class="form-switch">
                      <input
                        ...
                        @change="
                          (event) => changeCountryStatus(props.row.id, event)
                        "
                  ...
                  <div v-else-if="props.column.field == 'actions'">
                    ...
                    <AdminDeleteModal
                      ...
                      @deleteClicked="deleteCountry(props.row.id)"
...
export default {
  ...
  data() {
    return {
      columns: [
        ...
        {
          label: this.$t("columns.iso_code_2"),
          field: "iso_code_2",
        },
        {
          label: this.$t("columns.iso_code_3"),
          field: "iso_code_3",
        },
        {
  ...
  async fetch() {
    if (this.$gates.hasPermission("countries.index")) {
      await this.fetchCountries(this.page).then();
    }
  },
  methods: {
    ...mapActions({
      ...
      filterCountries: "Countries/filterCountries",
      updateCountryStatus: "Countries/updateCountryStatus",
      updateCountryIsDefault: "Countries/updateCountryIsDefault",
    }),
    async exportCountries(type) {
      this.filterData.export = type;
      await this.$repositories.countries.export(this.filterData);
    },
    ...
    async filter() {
      await this.filterCountries(this.filterData).then((response) => {
        if (response.state == "fail") {
          this.$toast.error(response.message);
        }
        if (response.state == "success") {
    ...
    async changeDefaultCountry(id, event) {
      await this.updateCountryIsDefault({
        filterData: this.filterData,
        country_id: id,
      }).then((response) => {
        if (response.state == "fail") {
    ...
    async changeCountryStatus(id, event) {
      await this.updateCountryStatus({
        filterData: this.filterData,
        country_id: id,
      })
        .then((response) => {
  ...
  mounted() {},
  watch: {
    allCountries(newCount, oldCount) {
      if (
        this.allCountries &&
        this.allCountries != null &&
        this.allCountries.data != null
      ) {
        this.rows = this.allCountries.data;
        this.totalRows = this.allCountries.meta.total;
        if (this.filterData.page != this.allCountries.meta.current_page) {
          this.filterData.page = this.allCountries.meta.current_page;
        }
        if (this.filterData.perPage != this.allCountries.meta.per_page) {
          this.filterData.perPage = this.allCountries.meta.per_page;
        }
      }
    },
  },
