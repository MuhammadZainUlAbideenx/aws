    ...
    <div class="content-header">
      <div class="row g-0">
        <div class="col-sm-12 px-0">
          <h2 class="m-0 text-body bold">{{ this.$t("sidebar.currency") }}</h2>
        </div>
        <!-- /.col -->
        <div class="row">
          <div class="col-sm-12 px-0">
            ...
            <p class="text-body-secondary">
              {{ this.$t("form.currency.index_description") }}
            </p>
          </div>
          <!-- /.col -->
        </div>
            ...
            <div class="row table-filter-row g-0">
              ...
              <div class="col-lg-4 table-actions px-0">
                <div slot="table-actions" class="align-middle">
                  <div class="share">
                    <nav>
                      <span @click="exportCurrencies('csv')">
                        <nuxt-link
                          to="#"
                      ...
                      <span @click="exportCurrencies('xlsx')">
                        <nuxt-link
                          to="#"
                      ...
                      <span @click="exportCurrencies('pdf')">
                        <nuxt-link
                          to="#"
                  ...
                  <nuxt-link
                    v-permission="'currencies.create'"
                    :to="localePath('/admin/currencies/create')"
                  >
                    <button
                      type="button"
                      v-tooltip="{ content: 'Add Currency' }"
                ...
                <template slot="table-row" slot-scope="props">
                  <div v-if="props.column.field == 'is_default'">
                    <div class="form-switch">
                      <input
                        ...
                        @change="
                          (event) => changeDefaultCurrency(props.row.id, event)
                        "
                  ...
                  <div v-else-if="props.column.field == 'is_active'">
                    <div class="form-switch">
                      <input
                        class="form-check-input"
                        @change="
                          (event) => changeCurrencyStatus(props.row.id, event)
                        "
                  ...
                  <div v-else-if="props.column.field == 'actions'">
                    <nuxt-link
                      ...
                      :to="localePath('/admin/currencies/view/' + props.row.id)"
                    >
                      <fa :icon="['fas', 'eye']" fixed-width />
                    </nuxt-link>
                    <nuxt-link
                      v-permission="'currencies.update'"
                      v-tooltip="{ content: 'Edit' }"
                      class="btn btn-sm rounded-circle text-success"
                      :to="localePath('/admin/currencies/edit/' + props.row.id)"
                    >
                      <fa :icon="['fas', 'edit']" fixed-width />
                    </nuxt-link>
                    <button
                      v-permission="'currencies.delete'"
                    ...
                    <AdminDeleteModal
                      ...
                      @deleteClicked="deleteCurrency(props.row.id)"
...
export default {
  ...
  async fetch() {
    if (this.$gates.hasPermission("currencies.index")) {
      await this.fetchCurrencies(this.page).then();
    }
  },
  methods: {
    ...mapActions({
      ...
      deleteCurrencies: "Currencies/deleteCurrencies",
      filterCurrencies: "Currencies/filterCurrencies",
      updateCurrencyStatus: "Currencies/updateCurrencyStatus",
      updateCurrencyIsDefault: "Currencies/updateCurrencyIsDefault",
    }),
    async exportCurrencies(type) {
      this.filterData.export = type;
      await this.$repositories.currencies.export(this.filterData);
    },
    ...
    async filter() {
      await this.filterCurrencies(this.filterData).then((response) => {
        if (response.state == "fail") {
          this.$toast.error(response.message);
        }
        if (response.state == "success") {
    ...
    async changeDefaultCurrency(id, event) {
      await this.updateCurrencyIsDefault({
        filterData: this.filterData,
        currency_id: id,
      }).then((response) => {
        if (response.state == "fail") {
    ...
    async changeCurrencyStatus(id, event) {
      await this.updateCurrencyStatus({
        filterData: this.filterData,
        currency_id: id,
      })
        .then((response) => {
  ...
  mounted() {},
  watch: {
    allCurrencies(newCount, oldCount) {
      if (
        this.allCurrencies &&
        this.allCurrencies != null &&
        this.allCurrencies.data != null
      ) {
        this.rows = this.allCurrencies.data;
        this.totalRows = this.allCurrencies.meta.total;
        if (this.filterData.page != this.allCurrencies.meta.current_page) {
          this.filterData.page = this.allCurrencies.meta.current_page;
        }
        if (this.filterData.perPage != this.allCurrencies.meta.per_page) {
          this.filterData.perPage = this.allCurrencies.meta.per_page;
        }
      }
    },
  },
