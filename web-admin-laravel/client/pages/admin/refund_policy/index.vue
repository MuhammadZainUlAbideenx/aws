        ...
        <div class="row g-0">
          <div class="col-sm-12 px-0">
            <h2 class="m-0 text-body bold">
              {{ this.$t("form.refund_policy.create_refund_policy") }}
            </h2>
          </div>
          <!-- /.col -->
          <div class="row">
            <div class="col-sm-12 px-0">
              <ol class="breadcrumb float-sm-right text-body">
                ...
                <li class="breadcrumb-item">
                  <nuxt-link :to="localePath('/admin/refund_policy')">{{ this.$t("sidebar.refund_policy") }}</nuxt-link>
                </li>
              ...
              <p class="text-body-secondary">
                {{ this.$t("form.refund_policy.form_description") }}
              </p>
            </div>
            <!-- /.col -->
          </div>
                          ...
                          <div class="tab-pane fade" :class="index == 0 ? 'active show':''" v-for="(language,index) in allActiveLanguages.languages" :id="language.code" :key="language.id" role="tabpanel" :aria-labelledby="language.code+'-tab'">
                              ...
                              <div class="row">
                                <div class="col-md-12 my-1" >
                                  <div role="group ">
                                    <label for="input-live" class=" form-label">{{ $t("form.refund_policy.label") }}:</label>
                                    ...
                                    <span class=" heebo-light">
                                        {{ $t("form.refund_policy.subheading") }}
                                      </span>
                                  </div>
                                </div>
                              </div>
                                ...
                                <div class="col-md-12 my-1">
                                  <div role="group ">
                                    <label
                                      ...
                                      >{{
                                        $t(
                                          "form.refund_policy.refund_policy.label"
                                        )
                                      }}:</label
                                    >
                                    <span
                                    ...
                                    >
                                      {{
                                        error["refund_policy." + language.code][0]
                                      }}
                                    </span>
                                    <GlobalCkeditorNuxt
                                      v-model="
                                    ...
                                    <span class="px-2 heebo-light">
                                      {{
                                        $t(
                                          "form.refund_policy.refund_policy.subheading"
                                        )
                                      }}
                                    </span>
                                  </div>
...
<script >
  ...
  export default {
    ...
    meta: {
      permission: 'refund_policy.create',
      strategy: 'create'
    },
    async fetch(){
        let data = await this.fetchRefundPolicy();
    ...
    methods: {
      ...mapActions({
        addRefundPolicy: 'RefundPolicy/addRefundPolicy',
        fetchRefundPolicy: 'RefundPolicy/fetchRefundPolicy',
      }),
      async add() {
        await this.addAndContinue()
        if (!this.error) {
          this.$router.replace(this.localePath('/admin/refund_policy'))
        }
      },
      async addAndContinue() {
        await this.addRefundPolicy(this.formData).then(res => {
          // some error occur
          if (res.response) {
