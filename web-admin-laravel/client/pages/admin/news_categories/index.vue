    ...
    <div class="content-header">
        <div class="row g-0">
          <div class="col-sm-12 px-0">
            <h2 class="m-0 text-body bold">{{this.$t("sidebar.news_category")}}</h2>
          </div>
          <!-- /.col -->
          <div class="row">
            <div class="col-sm-12 px-0">
              <ol class="breadcrumb float-sm-right text-body">
                ...
                <li class="breadcrumb-item active">{{this.$t("sidebar.news_category")}}</li>
              </ol>
              <p class="text-body-secondary">{{this.$t("form.news_category.index_description")}}</p>
            </div>
            <!-- /.col -->
          </div>
        </div>
             ...
             <div class="col-lg-8">
               ...
               <div class="col-lg-4 table-actions px-0">
                <div slot="table-actions" class="align-middle">
                  <div class="share">
                    <nav>
                      <span @click="exportNewsCategories('csv')">
                        <nuxt-link
                          to="#"
                      ...
                      <span @click="exportNewsCategories('xlsx')">
                        <nuxt-link
                          to="#"
                      ...
                      <span @click="exportNewsCategories('pdf')">
                        <nuxt-link
                          to="#"
                  ...
                  <nuxt-link
                    v-permission="'news_categories.create'"
                    :to="localePath('/admin/news_categories/create')"
                  >
                    <button
                      type="button"
                      v-tooltip="{ content: 'Add News Category' }"
                  ...
                  <div v-if="props.column.field == 'is_active'">
                    <div class="form-switch">
                      <input
                        ...
                        @change="
                          (event) => changeNewsCategoryStatus(props.row.id, event)
                        "
                   ...
                   <div v-else-if="props.column.field == 'actions'">
                    <nuxt-link
                            v-permission="'news_categories.index'" v-tooltip="{ content: 'Show' }"
                              class="btn btn-sm rounded-circle text-primary   "
                            :to="
                              localePath('/admin/news_categories/view/' + props.row.id)
                            "
                            >
                          ...
                          >
                          <nuxt-link
                          v-permission="'news_categories.update'"  v-tooltip="{ content: 'Edit' }"
                            class=" btn btn-sm rounded-circle text-success   "
                          :to="
                            localePath('/admin/news_categories/edit/' + props.row.id)
                          "
                          >
                            <fa :icon="['fas', 'edit']" fixed-width />
                          </nuxt-link>
                       <button
                        v-permission="'news_categories.delete'"
                        ...
                        >
                         <fa :icon="['fas', 'trash-alt']" fixed-width />
                                          ...
                                          <AdminDeleteModal :modal_id="'DeleteItem'+props.row.id" @deleteClicked="deleteNewsCategory(props.row.id)"  :delete_id="props.row.id">
                </AdminDeleteModal>
                  </div>
                  <span v-else>
...
<script >
  ...
  export default {
    ...
    meta: {
      permission: 'news_categories.index',
      strategy: 'read'
    },
    ...
    async fetch() {
      if (this.$gates.hasPermission('news_categories.index')) {
        await this.fetchNewsCategories(this.page).then()
      }
    },
    methods: {
      ...mapActions({
        fetchNewsCategories: 'NewsCategories/fetchNewsCategories',
        fetchActiveNewsCategories: 'General/fetchActiveNewsCategories',
        deleteNewsCategories: 'NewsCategories/deleteNewsCategories',
        filterNewsCategories: 'NewsCategories/filterNewsCategories',
        updateNewsCategoryStatus: 'NewsCategories/updateNewsCategoryStatus',
      }),
      async exportNewsCategories(type) {
        this.filterData.export = type
        await this.$repositories.news_categories.export(this.filterData)
      },
    async deleteNewsCategory(id) {
      await this.deleteNewsCategories({
      ...
      async filter() {
        await this.filterNewsCategories(this.filterData).then(response => {
          if (response.state == 'fail') {
            this.$toast.error(response.message)
          }
          if (response.state == 'success') {
    ...
    async changeNewsCategoryStatus(id, event) {
      await this.updateNewsCategoryStatus({
        filterData: this.filterData,
        news_category_id: id,
      }).then((response) => {
        if (response.state == "fail") {
    ...
    computed: {
      ...mapGetters({
        allNewsCategories: 'NewsCategories/allNewsCategories'
      })
    },
    mounted() {},
    watch: {
      allNewsCategories(newCount, oldCount) {
        if (this.allNewsCategories && this.allNewsCategories != null && this.allNewsCategories.data != null)   {
          this.rows = this.allNewsCategories.data
          this.totalRows = this.allNewsCategories.meta.total
          if (this.filterData.page != this.allNewsCategories.meta.current_page) {
            this.filterData.page = this.allNewsCategories.meta.current_page
          }
          if (this.filterData.perPage != this.allNewsCategories.meta.per_page) {
            this.filterData.perPage = this.allNewsCategories.meta.per_page
          }
        }
      }
    }
