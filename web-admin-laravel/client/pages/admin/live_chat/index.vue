              ...
              >
                <template slot="table-row" slot-scope="props">
                  ...
                  <div v-if="props.column.field == 'actions'">
                    <nuxt-link v-permission="'live_chat.index'"
                      class="btn btn-primary"
                      :to="localePath('/admin/live_chat/view/' + props.row.id)"
                    >{{$t('web.customer.orders.view_page.chat')}}
            </nuxt-link>
                    <!-- <nuxt-link
...
export default {
  ...
  async fetch() {
    await this.fetchAdminRiderMessages();
  },
  methods: {
    async fetchAdminRiderMessages() {
      const data = await this.$generalService.getAdminRiderMessages(this.filterData);
      if (data.success) {
        this.rows = data.data;
      } else {
