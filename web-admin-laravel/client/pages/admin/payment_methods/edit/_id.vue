        ...
        <div class="row g-0">
          <div class="col-sm-12 px-0">
            <h2 class="m-0 text-body bold">
              {{ $t("form.payment_method.edit_payment_method") }}
            </h2>
          </div>
          <!-- /.col -->
          <div class="row">
            <div class="col-sm-12 px-0">
              <ol class="breadcrumb float-sm-right text-body">
                ...
                <li class="breadcrumb-item">
                  <nuxt-link :to="localePath('/admin/payment_methods')">{{
                    $t("sidebar.payment_method")
                  }}</nuxt-link>
                </li>
              ...
              <p class="text-body-secondary">
                {{ $t("form.payment_method.edit_description") }}
              </p>
            </div>
            <!-- /.col -->
          </div>
                    ...
                    <div class="row">
                      <div class="col-md-6 my-1 ps-md-0">
                        <label class="label form-label">
                            {{ this.$t("form.payment_method.image.label") }}
                          </label>
                          ...
                          <div class="">
                            ...
                            <img v-if="payment_method_image_path" v-bind:src=" url + `${payment_method_image_path}`" class="" />
                            <div class="btn-media mb-3" type="button" name="button"  data-bs-toggle="modal" data-bs-target="#selectPaymentMethodMedia">{{ $t("form.select_media") }}</div>
                            <AdminMediaModal type="images" bind_modal="image_id" img_var="payment_method_image_path" modal_id="selectPaymentMethodMedia" @selectImage="imageSelected" redirect_panel="admin" />
                          </div>

                        <span class="heebo-light">
                            {{ $t("form.payment_method.image.subheading") }}
                          </span>
                      </div>
                      <div class="col-md-6 my-1 ps-md-0" v-if="ups_shipping_options.length > 0">
                    <div role="group" >
                      <label for="input-live" class="form-label"
                        >{{ $t("form.payment_method.shipping_service_type.label") }}:</label
                      >
                      <span
                      ...
                      <v-select
                        class=""
                        :placeholder="$t('form.payment_method.shipping_service_type.placeholder')"
                      ...
                      <span class="heebo-light">
                        {{ $t("form.payment_method.shipping_service_type.subheading") }}
                      </span>
                    </div>
                  </div>
                  <!-- These Checks are for (dnt generate empty divs) -->
                    <div class="col-md-6 my-1 pe-md-0" v-for="(setting,index) in formData.settings" :key="index" v-if="index != 'shipping_service_type_next_day_air' &&
                        ...
                        <div role="group" v-if="index != 'shipping_service_type_next_day_air' &&
                          ...
                          <span class="float-end text-danger"
                            ...
                            <v-select v-if="index == 'mode'"
                              class=""
                              :placeholder="$t('form.payment_method.mode.placeholder')"
                            ...
                            <v-select v-else-if="index == 'pickup_method'"
                              class=""
                              :placeholder="$t('form.payment_method.pickup_method.placeholder')"
                              v-model="formData.settings[index]"
                              :reduce="(opt) => opt.value"
                              label="value"
                          ...
                          <input v-else :class="error && error[`settings.${index}`] ? 'error' : ''"
                                 class="form-control"
                        ...
                        <div role="group" v-else-if="index == 'country'">
                          ...
                          <span class="float-end text-danger"
                            ...
                            <AdminSearchSelectable :class="error && error[`settings.${index}`] ? 'error' : ''" apiMethod="activeCountries" responseKey="countries"
                              :initialOptions="allActiveCountries.countries" :selectedOptions="selected_country" :placeholder="$t('form.payment_method.country.placeholder')" v-model="formData.settings[index]" v-on:input="getStatesByCountry($event)" />
                         <span class=" heebo-light">
                             {{ index }}
                         </span>
                        </div>
                        <div role="group" v-else-if="index == 'state'">
                          ...
                          <span class="float-end text-danger"
                            ...
                            <AdminSearchSelectable :class="error && error[`settings.${index}`] ? 'error' : ''" :search_id="formData.settings[index]" apiMethod="getStatesByCountry" responseKey="states"
                              :initialOptions="statesByCountry.states" :selectedOptions="selected_state" :placeholder="$t('form.payment_method.state.placeholder')" :key="states_key" v-model="formData.settings[index]" v-on:input="getCitiesByState($event)" />
                         <span class=" heebo-light">
                             {{ index }}
                         </span>
                        </div>
                        <div role="group" v-else-if="index == 'city'">
                          ...
                          <span class="float-end text-danger"
                            ...
                            <AdminSearchSelectable :class="error && error[`settings.${index}`] ? 'error' : ''" :search_id="formData.settings[index]" apiMethod="getCitiesByState" responseKey="cities"
                              :initialOptions="citiesByState.cities" :selectedOptions="selected_city" :placeholder="$t('form.payment_method.city.placeholder')" :key="cities_key" v-model="formData.settings[index]" />
                         <span class=" heebo-light">
                             {{ index }}
                         </span>
                        </div>
                  ...
                  <div class="col-md-6 my-1 ps-md-0">
                    <div class="d-flex align-items-center">
                      <label class="label form-label me-4 text-capitalize">
                        {{ $t("form.payment_method.status.label") }}
                      </label>
                    ...
                    <span class="heebo-light">
                      {{ $t("form.payment_method.status.subheading") }}
                    </span>
                  </div>
                  <div class="col-md-6 my-1 pe-md-0">
                    <div class="d-flex align-items-center">
                      <label class="label form-label me-4 text-capitalize">
                        {{ $t("form.payment_method.is_default.label") }}
                      </label>
                    ...
                    <span class="heebo-light">
                      {{ $t("form.payment_method.is_default.subheading") }}
                    </span>
                  </div>
                </div>
                              ...
                              <div class="row">
                                <div class="col-md-6 my-1">
                                  <div role="group ">
                                    <label for="input-live" class="form-label"
                                      >{{
                                        $t("form.payment_method.name.label")
                                      }}:</label
                                    >
                                    <span
                                      class="float-end text-danger"
                                    ...
                                    <input
                                      ...
                                      aria-describedby="input-live-help input-live-feedback"
                                      :placeholder="
                                        $t('form.payment_method.name.placeholder')
                                      "
                                      trim
                                    />
                                    <span class="heebo-light">
                                      {{
                                        $t("form.payment_method.name.subheading")
                                      }}
                                    </span>
                                  </div>
                                </div>
                                ...
                                <div class="col-md-12 my-1">
                                  <div role="group ">
                                    <label
                                      ...
                                      >{{
                                        $t(
                                          "form.payment_method.description.label"
                                        )
                                      }}:</label
                                    >
                                    <span
                                    ...
                                    <span class="px-2 heebo-light">
                                      {{
                                        $t(
                                          "form.payment_method.description.subheading"
                                        )
                                      }}
                                    </span>
                                  </div>
...
export default {
  ...
  data() {
    return {
      ...
      cities_key:1,
      mode_options: [
        { value: '', text: this.$t('form.payment_method.mode.placeholder') },
        { value: 'test', text: this.$t('form.payment_method.mode.test') },
        { value: 'production', text: this.$t('form.payment_method.mode.production') },
      ],
      pickup_methods_options: [
        { value: '', text: this.$t('form.payment_method.pickup_method.placeholder') },
        { value: 'daily', text: this.$t('form.payment_method.pickup_method.daily') },
        { value: 'cutomer_counter', text: this.$t('form.payment_method.pickup_method.cutomer_counter') },
        { value: 'one_time', text: this.$t('form.payment_method.pickup_method.one_time') },
        { value: 'on_call_air', text: this.$t('form.payment_method.pickup_method.on_call_air') },
        { value: 'letter_center', text: this.$t('form.payment_method.pickup_method.letter_center') },
        { value: 'air_service_center', text: this.$t('form.payment_method.pickup_method.air_service_center') },
      ],
      url: '/backend',
  ...
  async fetch() {
    ...
    const { data } = await this.$repositories.payment_methods.show(
      this.$route.params.id
    );
    if(data.image){
      this.formData.image_id = data.image.id
      if(data.image.thumbnails){
        this.payment_method_image_path = data.image.thumbnails.small.thumbnail
      }
    }
    // get and update data
    ...
    methods: {
    ...mapActions({
      updatePaymentMethods: "PaymentMethods/updatePaymentMethods",
      fetchActivePaymentMethods: "General/fetchActivePaymentMethods",
    ...
    async update() {
      ...
      if (!this.error) {
        this.$router.replace(this.localePath("/admin/payment_methods"));
      }
    },
    async updateAndContinue() {
      ...
      await this.updatePaymentMethods({
        ...
        if (res.response) {
          ...
          this.fetchActivePaymentMethods();
        }
      });
        this.disabled=false
    },
