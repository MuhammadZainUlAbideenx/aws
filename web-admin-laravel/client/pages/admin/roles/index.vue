    ...
    <div class="content-header">
      <div class="row g-0">
        <div class="col-sm-12 px-0">
          <h2 class="m-0 text-body bold">{{ this.$t("sidebar.role") }}</h2>
        </div>
        <!-- /.col -->
        <div class="row">
          <div class="col-sm-12 px-0">
            ...
            <p class="text-body-secondary">
              {{ this.$t("form.role.index_description") }}
            </p>
          </div>
          <!-- /.col -->
        </div>
              ...
              <div class="col-lg-4 table-actions px-0">
                <div slot="table-actions" class="align-middle">
                  ...
                  <nuxt-link
                    ...
                    :to="localePath('/admin/roles/create')"
                  >
                    <button
                      type="button"
                      v-tooltip="{ content: 'Add Role' }"
                ...
                <template slot="table-row" slot-scope="props">
                  <div v-if="props.column.field == 'is_active'">
                    <div class="form-switch">
                      <input
                        ...
                        @change="
                          (event) => changeRoleStatus(props.row.id, event)
                        "
                  ...
                  <div v-else-if="props.column.field == 'actions'">
                    <nuxt-link
                      ...
                      :to="localePath('/admin/roles/view/' + props.row.id)"
                      ><button
                        type="button"
                    ...
                    <nuxt-link
                      ...
                      :to="localePath('/admin/roles/edit/' + props.row.id)"
                      ><button
                        type="button"
                      ...
                      <nuxt-link
                        ...
                        :to="
                          localePath(
                            '/admin/roles/edit/' + props.row.id + '/permissions'
                          )
                        "
                        ><button
                          type="button"
                          v-tooltip="{ content: 'Permission' }"
                          class="
                            btn btn-sm
                            rounded-circle
                          ...
                          name="button"
                        >
                          <fa :icon="['fas', 'key']" fixed-width /></button
                      ></nuxt-link>
                    </nuxt-link>
                    <button
                    ...
                    <AdminDeleteModal
                      ...
                      @deleteClicked="deleteRole(props.row.id)"
...
export default {
  ...
  async fetch() {
    if (this.$gates.hasPermission("roles.index")) {
      await this.fetchRoles(this.page).then();
    }
  },
  methods: {
    ...mapActions({
      fetchRoles: "Roles/fetchRoles",
      deleteRoles: "Roles/deleteRoles",
      filterRoles: "Roles/filterRoles",
      updateRoleStatus: "Roles/updateRoleStatus",
      fetchActiveRoles: "General/fetchActiveRoles",
    }),
    async exportRoles(type) {
      this.filterData.export = type;
      await this.$repositories.roles.export(this.filterData);
    },
    async deleteRole(id) {
    ...
    async filter() {
      await this.filterRoles(this.filterData).then((response) => {
        if (response.state == "fail") {
          this.$toast.error(response.message);
        }
        if (response.state == "success") {
    ...
    async changeRoleStatus(id, event) {
  ...
  mounted() {},
  watch: {
    allRoles(newCount, oldCount) {
      if (
        this.allRoles &&
        this.allRoles != null &&
        this.allRoles.data != null
      ) {
        this.rows = this.allRoles.data;
        this.totalRows = this.allRoles.meta.total;
        if (this.filterData.page != this.allRoles.meta.current_page) {
          this.filterData.page = this.allRoles.meta.current_page;
        }
        if (this.filterData.perPage != this.allRoles.meta.per_page) {
          this.filterData.perPage = this.allRoles.meta.per_page;
        }
      }
    },
  },
