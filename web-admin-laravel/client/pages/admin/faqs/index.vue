    ...
    <div class="content-header">
        <div class="row g-0">
          <div class="col-sm-12 px-0">
            <h2 class="m-0 text-body bold">{{this.$t("sidebar.faqs")}}</h2>
          </div>
          <!-- /.col -->
          <div class="row">
            <div class="col-sm-12 px-0">
              <ol class="breadcrumb float-sm-right text-body">
                ...
                <li class="breadcrumb-item active">{{this.$t("sidebar.faqs")}}</li>
              </ol>
              <p class="text-body-secondary">{{this.$t("form.faqs.index_description")}}</p>
            </div>
            <!-- /.col -->
          </div>
        </div>
        ...
        <div class="card gutter-b border-0" >
         ...
         <div class="row table-filter-row g-0">
          <!-- Small boxes (Stat box) -->
             <div class="col-lg-8">
                 <div class="d-flex flex-column flex-md-row"> <select class="form-select custom-select"
               ...
               <div class="col-lg-4 table-actions px-0">
                <div slot="table-actions" class="align-middle">
                  ...
                  <nuxt-link
                    v-permission="'faqs.create'"
                    :to="localePath('/admin/faqs/create')"
                  >
                    <button
                      type="button"
                      v-tooltip="{ content: 'Add Faq' }"
                  ...
                  <div v-if="props.column.field == 'is_active'">
                    <div class="form-switch">
                      <input
                        ...
                        @change="
                          (event) => changeFaqStatus(props.row.id, event)
                        "
                   ...
                   <div v-else-if="props.column.field == 'is_featured'">
                    ...
                    <div v-else-if="props.column.field == 'answer'">
                        <p v-html="props.row.answer"></p>
                      <!-- <label class="form-check-label" for="flexSwitchCheckChecked">Checked switch checkbox input</label> -->
                    </div>

                   <div v-else-if="props.column.field == 'actions'">
                    <nuxt-link
                            v-permission="'faqs.index'" v-tooltip="{ content: 'Show' }"
                              class="btn btn-sm rounded-circle text-primary   "
                            :to="
                              localePath('/admin/faqs/view/' + props.row.id)
                            "
                            >
                          ...
                          >
                          <nuxt-link
                          v-permission="'faqs.update'" v-tooltip="{ content: 'Edit' }"
                            class=" btn btn-sm rounded-circle text-success   "
                          :to="
                            localePath('/admin/faqs/edit/' + props.row.id)
                          "
                          >
                            <fa :icon="['fas', 'edit']" fixed-width />
                          </nuxt-link>
                       <button
                        v-permission="'faqs.delete'"
                        ...
                        >
                         <fa :icon="['fas', 'trash-alt']" fixed-width />
                                          ...
                                          <AdminDeleteModal :modal_id="'DeleteItem'+props.row.id" @deleteClicked="deleteFaq(props.row.id)"  :delete_id="props.row.id">
                </AdminDeleteModal>
                  </div>
                  <span v-else>
  ...
  export default {
    ...
    data() {
      return {
        ...
        columns: [
          {
            label: this.$t('columns.question'),
            field: 'question'
          },
           {
            label: this.$t('columns.answer'),
            field: 'answer',
            sortable: false
          },
          {
    ...
    async fetch() {
      if (this.$gates.hasPermission('faqs.index')) {
        await this.fetchFaqs(this.page).then()
      }
    },
    methods: {
      ...mapActions({
        fetchFaqs: 'Faqs/fetchFaqs',
        fetchActiveFaqs: 'General/fetchActiveFaqs',
        deleteFaqs: 'Faqs/deleteFaqs',
        filterFaqs: 'Faqs/filterFaqs',
        updateFaqStatus: 'Faqs/updateFaqStatus',
      }),
      async exportFaqs(type) {
        this.filterData.export = type
        await this.$repositories.faqs.export(this.filterData)
      },
    async deleteFaq(id) {
      await this.deleteFaqs({
      ...
      async filter() {
        await this.filterFaqs(this.filterData).then(response => {
          if (response.state == 'fail') {
            this.$toast.error(response.message)
          }
          if (response.state == 'success') {
    ...
    async changeFaqStatus(id, event) {
    ...
    computed: {
      ...mapGetters({
        allFaqs: 'Faqs/allFaqs'
      })
    },
    mounted() {},
    watch: {
      allFaqs(newCount, oldCount) {
        if (this.allFaqs && this.allFaqs != null && this.allFaqs.data != null)   {
          this.rows = this.allFaqs.data
          this.totalRows = this.allFaqs.meta.total
          if (this.filterData.page != this.allFaqs.meta.current_page) {
            this.filterData.page = this.allFaqs.meta.current_page
          }
          if (this.filterData.perPage != this.allFaqs.meta.per_page) {
            this.filterData.perPage = this.allFaqs.meta.per_page
          }
        }
      }
    }
