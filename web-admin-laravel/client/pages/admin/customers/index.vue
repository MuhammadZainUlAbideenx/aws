    ...
    <div class="content-header">
      <div class="row g-0">
        <div class="col-sm-12 px-0">
          <h2 class="m-0 text-body bold">{{ this.$t("sidebar.customer") }}</h2>
        </div>
        <!-- /.col -->
        <div class="row">
          <div class="col-sm-12 px-0">
            ...
            <p class="text-body-secondary">
              {{ this.$t("form.customer.index_description") }}
            </p>
          </div>
          <!-- /.col -->
        </div>
              ...
              <div class="col-lg-4 table-actions px-0">
                <div slot="table-actions" class="align-middle">
                  ...
                  <nuxt-link
                    ...
                    :to="localePath('/admin/customers/create')"
                  >
                    <button
                      type="button"
                      v-tooltip="{ content: 'Add Customers' }"
              ...
              >
                <template slot="table-row" slot-scope="props">
                  <div v-if="props.column.field == 'is_active'">
                    <div class="form-switch">
                      <input
                        ...
                        @change="
                          (event) => changeCustomerStatus(props.row.id, event)
                        "
                  ...
                  <div v-else-if="props.column.field == 'actions'">
                    <nuxt-link
                      v-permission="'customers.index'"
                      :to="localePath('/admin/customers/view/' + props.row.id)"
                      ><button
                        type="button"
                      ...
                      >
                        <fa :icon="['fas', 'eye']" fixed-width /></button
                    ></nuxt-link>
                    <nuxt-link
                      v-permission="'customers.update'"
                      class="btn btn-sm rounded-circle text-success"
                      v-tooltip="{ content: 'Edit' }"
                      :to="localePath('/admin/customers/edit/' + props.row.id)"
                    >
                      <fa :icon="['fas', 'edit']" fixed-width />
                    </nuxt-link>
                    <nuxt-link
                      v-permission="'addresses.index'"
                      v-tooltip="{ content: 'Manage address' }"
                      class="btn btn-sm rounded-circle text-success"
                      :to="
                        localePath('/admin/addresses/customer/' + props.row.id)
                      "
                    >
                      <fa :icon="['fas', 'map']" fixed-width />
                    </nuxt-link>
                    <button
                    ...
                    <AdminDeleteModal
                      ...
                      @deleteClicked="deleteCustomer(props.row.id)"
...
export default {
  ...
  async fetch() {
    if (this.$gates.hasPermission("customers.index")) {
      await this.fetchCustomers(this.page).then();
    }
  },
  methods: {
    ...mapActions({
      ...
      updateCustomerIsDefault: "Customers/updateCustomerIsDefault",
    }),
    async exportCustomers(type) {
      this.filterData.export = type;
      await this.$repositories.customers.export(this.filterData);
    },
    ...
    async filter() {
      await this.filterCustomers(this.filterData).then((response) => {
        if (response.state == "fail") {
          this.$toast.error(response.message);
        }
        if (response.state == "success") {
    ...
    async changeDefaultCustomer(id, event) {
      await this.updateCustomerIsDefault({
        filterData: this.filterData,
        customer_id: id,
      }).then((response) => {
        if (response.state == "fail") {
    ...
    async changeCustomerStatus(id, event) {
  ...
  mounted() {},
  watch: {
    allCustomers(newCount, oldCount) {
      if (
        this.allCustomers &&
        this.allCustomers != null &&
        this.allCustomers.data != null
      ) {
        this.rows = this.allCustomers.data;
        this.totalRows = this.allCustomers.meta.total;
        if (this.filterData.page != this.allCustomers.meta.current_page) {
          this.filterData.page = this.allCustomers.meta.current_page;
        }
        if (this.filterData.perPage != this.allCustomers.meta.per_page) {
          this.filterData.perPage = this.allCustomers.meta.per_page;
        }
      }
    },
  },
