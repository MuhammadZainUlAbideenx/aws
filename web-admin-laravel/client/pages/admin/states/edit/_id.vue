          ...
          <div class="row">
            <div class="col-sm-12 px-0">
              ...
              <p class="text-body-secondary">
                {{ this.$t("form.state.edit_description") }}
              </p>
            </div>
            <!-- /.col -->
          </div>
                    ...
                    <div role="group">
                      ...
                      <span class="float-end text-danger"
                        ...
                        <AdminSearchSelectable :class="error && error.country_id ? 'error' : ''" apiMethod="activeCountries" responseKey="countries"
                          :initialOptions="allActiveCountries.countries" :selectedOptions="selected_country" v-model="formData.country_id" v-on:input="getZonesByCountry($event)" />
...
<script >
  ...
  export default {
    ...
    async fetch() {
      ...
      if(data.country){
        await this.setZonesByCountry(data.country.id)
        this.formData.country_id = data.country.id
        this.selected_country = data.country
      }
      if(data.zone){
    ...
    methods: {
      ...
      async setZonesByCountry(id){
          const data =  await this.$generalService.getZonesByCountry(id);
          if(data.zones){
          this.zonesByCountry = data.zones;
            }
          this.$forceUpdate();
      ...
      ...mapActions({
        updateStates: 'States/updateStates',
