    ...
    <div class="content-header">
        <div class="row g-0">
          <div class="col-sm-12 px-0">
            <h2 class="m-0 text-body bold">{{this.$t("sidebar.state")}}</h2>
          </div>
          <!-- /.col -->
          <div class="row">
            <div class="col-sm-12 px-0">
              <ol class="breadcrumb float-sm-right text-body">
                ...
                <li class="breadcrumb-item active">{{this.$t("sidebar.state")}}</li>
              </ol>
              <p class="text-body-secondary">{{this.$t("form.state.index_description")}}</p>
            </div>
            <!-- /.col -->
          </div>
        </div>
             ...
             <div class="col-lg-8">
               ...
               <div class="col-lg-4 table-actions px-0">
                <div slot="table-actions" class="align-middle">
                  ...
                  <nuxt-link
                    v-permission="'states.create'"
                    :to="localePath('/admin/states/create')"
                  >
                    <button
                      type="button"
                      v-tooltip="{ content: 'Add State' }"
                ...
                <div v-if="props.column.field == 'is_active'">
                  <div class="form-switch">
                    <input
                      ...
                      @change="
                        (event) => changeStateStatus(props.row.id, event)
                      "
                 ...
                 <div v-else-if="props.column.field == 'actions'">
                    <nuxt-link
                            v-permission="'states.index'"  v-tooltip="{ content: 'Show' }"
                              class="btn btn-sm rounded-circle text-primary   "
                            :to="
                              localePath('/admin/states/view/' + props.row.id)
                            "
                            >
                          ...
                          >
                          <nuxt-link
                          v-permission="'states.update'" v-tooltip="{ content: 'Edit' }"
                            class=" btn btn-sm rounded-circle text-success   "
                          :to="
                            localePath('/admin/states/edit/' + props.row.id)
                          "
                          >
                            <fa :icon="['fas', 'edit']" fixed-width />
                          </nuxt-link>
                       <button
                        v-permission="'states.delete'"
                        ...
                        <AdminDeleteModal :modal_id="'DeleteItem'+props.row.id" @deleteClicked="deleteState(props.row.id)"  :delete_id="props.row.id">
                          </AdminDeleteModal>
                  </div>
                  <span v-else>
...
<script >
  ...
  export default {
    ...
    async fetch() {
      if (this.$gates.hasPermission('states.index')) {
        await this.fetchStates(this.page).then()
      }
    },
    methods: {
      ...mapActions({
        ...
        deleteStates: 'States/deleteStates',
        filterStates: 'States/filterStates',
        updateStateStatus: 'States/updateStateStatus',
        updateStateIsDefault: 'States/updateStateIsDefault'
      }),
      async exportStates(type) {
        this.filterData.export = type
        await this.$repositories.states.export(this.filterData)
      },
    async deleteState(id) {
      ...
      async filter() {
        await this.filterStates(this.filterData).then(response => {
          if (response.state == 'fail') {
            this.$toast.error(response.message)
          }
          if (response.state == 'success') {
      ...
      async changeDefaultState(id, event) {
        await this.updateStateIsDefault({
          filterData: this.filterData,
          state_id: id
        }).then(response => {
          if (response.state == 'fail') {
    ...
    async changeStateStatus(id, event) {
    ...
    mounted() {},
    watch: {
      allStates(newCount, oldCount) {
        if (this.allStates && this.allStates != null && this.allStates.data != null)   {
          this.rows = this.allStates.data
          this.totalRows = this.allStates.meta.total
          if (this.filterData.page != this.allStates.meta.current_page) {
            this.filterData.page = this.allStates.meta.current_page
          }
          if (this.filterData.perPage != this.allStates.meta.per_page) {
            this.filterData.perPage = this.allStates.meta.per_page
          }
        }
      }
    }
