    ...
    >
      <div class="py-4 px-6">
        ...
        >
          ...
          <span class="ml-3 text-xl">{{$t('tailblocks')}}</span>
        </nuxt-link>

        <p class="text-xl text-gray-600 text-center">{{$t('set_your_password')}}</p>
...
export default {
  ...
  methods: {
    async setPassword() {
      ...
      await this.$axios
        .$post(
          `/api/set-password/${this.user}?expires=${this.$route.query.expires}&signature=${this.$route.query.signature}`,
          this.form
        )
        .then(() => {
          this.$toast.success(this.$t('password_was_set_successfully'))
