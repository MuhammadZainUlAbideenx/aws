          ...
          <div class="row">
            <div class="col-sm-12 px-0">
              <ol class="breadcrumb float-sm-right text-body">
                ...
                <li class="breadcrumb-item active">{{this.$t("sidebar.review")}}</li>
              </ol>
              <p class="text-body-secondary">{{this.$t("form.review.index_description")}}</p>
            </div>
            <!-- /.col -->
          </div>
        </div>
              ...
              <div class="col-lg-4 table-actions px-0">
                <div slot="table-actions" class="align-middle">
                  ...
                  <nuxt-link
                    ...
                    :to="localePath('/seller/reviews/create')"
                  >
                    <button
                      type="button"
                ...
                <div v-if="props.column.field == 'is_active'">
                  ...
                  <div v-else-if="props.column.field == 'actions'">
                    <nuxt-link
                            ...
                            :to="
                              localePath('/seller/reviews/view/' + props.row.id)
                            "
                            >
                          ...
                          :to="
                            localePath('/seller/reviews/edit/' + props.row.id)
                          "
                          >
                       ...
                       <button
                        ...
                        >
                         <fa :icon="['fas', 'trash-alt']" fixed-width />
                                          ...
                                          <AdminDeleteModal :modal_id="'DeleteItem'+props.row.id" @deleteClicked="deleteReview(props.row.id)"  :delete_id="props.row.id">
                </AdminDeleteModal>
                  </div>
                  <span v-else>
...
<script >
  ...
  export default {
    ...
    methods: {
      ...mapActions({
        fetchReviews: 'Seller/Reviews/fetchReviews',
        fetchActiveReviews: 'General/fetchActiveReviews',
        deleteReviews: 'Seller/Reviews/deleteReviews',
        filterReviews: 'Seller/Reviews/filterReviews',
        updateReviewStatus: 'Seller/Reviews/updateReviewStatus',
        updateReviewIsDefault: 'Seller/Reviews/updateReviewIsDefault'
      }),
      async exportReviews(type) {
        this.filterData.export = type
        await this.$sellerRepositories.reviews.export(this.filterData)
      },
    ...
    computed: {
      ...mapGetters({
        allReviews: 'Seller/Reviews/allReviews'
      })
    },
    mounted() {},
    watch: {
      allReviews(newCount, oldCount) {
        if (this.allReviews && this.allReviews != null && this.allReviews.data != null)   {
