<template >
          ...
          <div class="row">
            <div class="col-sm-12 px-0">
              <ol class="breadcrumb float-sm-right text-body">
                ...
                <li class="breadcrumb-item active">{{this.$t("sidebar.order")}}</li>
              </ol>
              <p class="text-body-secondary">{{this.$t("order.index_description")}}</p>
            </div>
            <!-- /.col -->
          </div>
        </div>
  ...
  <select class="form-select custom-select"
                          ...
                          <option value="order_number">{{ $t('columns.number')}}</option>
                       </select>
              ...
              <div class="col-lg-4 table-actions px-0" >
                ...
                <!--  <nuxt-link
                    ...
                    :to="localePath('/seller/orders/create')"
                  >
                    <button
                      type="button"
                  ...
                  <div v-else-if="props.column.field == 'actions'">
                    <nuxt-link
                           v-tooltip="{ content: 'Show' }"
                            ...
                            :to="
                              localePath('/seller/orders/view/' + props.row.id)
                            "
                            >
                              <fa :icon="['fas', 'eye']" fixed-width /></nuxt-link
                          >

                          <!-- <nuxt-link
                          v-permission="'orders.update'"  v-tooltip="{ content: 'Edit' }"
                            class=" btn btn-sm rounded-circle text-success   "
                          :to="
                            localePath('/seller/orders/edit/' + props.row.id)
                          "
                          >
                       ...
                       <!-- <button
                        ...
                        >
                         <fa :icon="['fas', 'trash-alt']" fixed-width />
                                          ...
                                          <AdminDeleteModal :modal_id="'DeleteItem'+props.row.id" @deleteClicked="deleteOrder(props.row.id)"  :delete_id="props.row.id">
                </AdminDeleteModal> -->
                  </div>
...
<script >
  ...
  export default {
    ...
    methods: {
      ...mapActions({
        fetchOrders: 'Seller/Orders/fetchOrders',
        fetchActiveOrders: 'General/fetchActiveOrders',
        deleteOrders: 'Seller/Orders/deleteOrders',
        filterOrders: 'Seller/Orders/filterOrders',
        updateOrderStatus: 'Seller/Orders/updateOrderStatus',
        updateOrderIsDefault: 'Seller/Orders/updateOrderIsDefault'
      }),
      async exportOrders(type) {
        this.filterData.export = type
        await this.$sellerRepositories.orders.export(this.filterData)
      },
    ...
    computed: {
      ...mapGetters({
        allOrders: 'Seller/Orders/allOrders'
      })
    },

    watch: {
      allOrders(newCount, oldCount) {
        if (this.allOrders && this.allOrders != null && this.allOrders.data != null)   {
