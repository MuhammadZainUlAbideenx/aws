          ...
          <div class="row">
            <div class="col-sm-12 px-0">
              <ol class="breadcrumb float-sm-right text-body">
                ...
                <li class="breadcrumb-item active">{{this.$t("sidebar.product")}}</li>
              </ol>
              <p class="text-body-secondary">{{this.$t("form.product.index_description")}}</p>
            </div>
            <!-- /.col -->
          </div>
        </div>
              ...
              <div class="col-lg-8">
                ...
                <div slot="table-actions" class="align-middle">
                  ...
                  <nuxt-link
                    ...
                    :to="localePath('/seller/products/create')"
                  >
                    <button
                      type="button"
            ...
            <vue-good-table v-if="!$fetchState.pending"
              ...
              <template slot="table-row" slot-scope="props">
               ...
               <div v-if="props.column.field == 'is_active'">
                    ...
                    <div class="form-switch" v-else-if="props.row.product_type == 2 && props.row.variants.length == 0 ">
                        <input
                        class="form-check-input"
                        disabled
                        @change="
                        ...
                        id="flexSwitchCheckChecked"
                      />
                      <span  v-tooltip="{ content: 'Please add variants' }"><fa  :icon="['fas', 'info-circle']" fixed-width /></span>
                    ...
                    <div class="form-switch"  v-else>
                     ...
                     <div v-else-if="props.column.field == 'display_price'">
...
<!-- sale and flash sale tag with the name starts -->
    ...
    <div v-else-if="props.column.field == 'name'">
                    <span v-if="props.row.special_sale">
                      {{ props.row.name }} <span class="badge bg-danger">Special Sale</span>
                    </span>
                    <span v-else-if="props.row.flash_sale">
                      {{ props.row.name }} <span class="badge bg-danger">Flash Sale</span>
                    </span>
                    <span v-else>
                      {{ props.row.name }}
                    </span>
                  ...
                  <div v-else-if="props.column.field == 'product_type'">
                    ...
                    <nuxt-link
                            ...
                            :to="
                              localePath('/seller/products/view/' + props.row.id)
                            "
                            >
                          ...
                          :to="
                            localePath('/seller/products/edit/' + props.row.id)
                          "
                          >
                       ...
                       <button
                        ...
                        >
                         <fa :icon="['fas', 'trash-alt']" fixed-width />
                                           ...
                                           <AdminDeleteModal :modal_id="'DeleteItem'+props.row.id" @deleteClicked="deleteProduct(props.row.id)"  :delete_id="props.row.id">
                </AdminDeleteModal>
                  </div>
                  <span v-else>
...
<script >
  ...
  export default {
    ...
    methods: {
      ...mapActions({
        fetchProducts: 'Seller/Products/fetchProducts',
        deleteProducts: 'Seller/Products/deleteProducts',
        filterProducts: 'Seller/Products/filterProducts',
        updateProductStatus: 'Seller/Products/updateProductStatus',
        fetchActiveProducts: 'General/fetchActiveProducts',
      }),
      async exportProducts(type) {
        this.filterData.export = type
        await this.$sellerRepositories.products.export(this.filterData)
      },
    ...
    computed: {
      ...mapGetters({
        allProducts: 'Seller/Products/allProducts'
      })
    },
    watch: {
      allProducts(newCount, oldCount) {
        if (this.allProducts && this.allProducts != null && this.allProducts.data != null)   {
